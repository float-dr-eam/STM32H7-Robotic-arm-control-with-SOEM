    ...
    if (ec_init(ifname))
    {
        ...
        if (ec_config_init(FALSE) > 0)
        {
            ...
            if (ec_slave[0].state == EC_STATE_OPERATIONAL)
            {
                ...
                //read_max_velocity();
                // set_max_velocity( 1, 120);
                // set_max_velocity( 2, 1200);
                // 激活循环过程数据
                dorun = 1;
            }
            else
...
//slave_index 1-7
void set_max_velocity(uint8 slave_index, int32 target_vel)//target_vel    r/min(CSP模式)
{
    if (slave_index <= ec_slavecount)
    {
        // 设置目标速度
        ec_SDOwrite(slave_index, 0x6080, 0x00, FALSE, sizeof(target_vel), &target_vel, EC_TIMEOUTRXM);
        printf("Motor %d max velocity set to %d\n", slave_index, target_vel);
    }
    else
    {
        printf("Invalid slave index: %d\n", slave_index);
...
void read_max_velocity(void)
{
    uint8 cnt;
    for (cnt = 1; cnt <= ec_slavecount; cnt++)
    {
        int readlen = sizeof(LD3M_all.max_velocity[cnt - 1]);
        ec_SDOread(cnt, 0x6080, 0x00, 0, &readlen, &LD3M_all.max_velocity[cnt - 1], EC_TIMEOUTRXM);
    }
}
void set_max_acceleration(uint8 slave_index, int32 target_acc)//target_acc  Unit/s^2(PPM模式)
{
    if (slave_index <= ec_slavecount)
    {
        // 设置目标加速度
        ec_SDOwrite(slave_index, 0x6083, 0x00, FALSE, sizeof(target_acc), &target_acc, EC_TIMEOUTRXM);
        printf("Motor %d max acceleration set to %d\n", slave_index, target_acc);
    }
    else
    {
        printf("Invalid slave index: %d\n", slave_index);
...
void set_max_deceleration(uint8 slave_index, int32 target_decc)//target_decc  Unit/s^2(PPM模式)
{
    if (slave_index <= ec_slavecount)
    {
        // 设置目标减速度
        ec_SDOwrite(slave_index, 0x6084, 0x00, FALSE, sizeof(target_decc), &target_decc, EC_TIMEOUTRXM);
        printf("Motor %d max deceleration set to %d\n", slave_index, target_decc);
    }
    else
    {
        printf("Invalid slave index: %d\n", slave_index);
...
void get_cur_degree(void)
{
    for (uint8 cnt = 0; cnt < ec_slavecount; cnt++)
    {
        LD3M_all.cur_degree[cnt] = (my_float)(LD3M_all.cur_pos[cnt] - origin_point[cnt + 1]) * rotate_sign[cnt] / UNIT_PULSES[cnt]; // 计算当前角度 °
    }
}

/**
 * @brief 计算目标位置和当前位置的差距
 * @return int32 位置差值
 */
int32 get_position_difference(void) 
{
    int32 position_diff = 0;
   for (uint8 cnt = 0; cnt < ec_slavecount-1; cnt++)// ec_slavecount-1 -》 0-5   ec_slavecount -》 0-
   {
    if (LD3M_all.target_pos[cnt] < LD3M_all.cur_pos[cnt])
    {
        position_diff += (LD3M_all.cur_pos[cnt] - LD3M_all.target_pos[cnt]);
    }
    else
    {
        position_diff += (LD3M_all.target_pos[cnt] - LD3M_all.cur_pos[cnt]);
    }
   }
...
void LD3M_loop(void)
{
    ...
    for (cnt = 0; cnt < ec_slavecount; cnt++)
    {
        ...
        {
            // 初始化轨迹规划参数
            //LD3M_all.trajectory_time[cnt] = 0;
            LD3M_all.LD3Mflag[cnt] = 1;
            LD3M_all.has_new_target[cnt] = false;
        }
        // 如果LD3M从站状态标志为 1
