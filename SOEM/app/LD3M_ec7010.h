...
typedef struct
{
   ...
   int32 max_velocity[NUM_SLAVES];  // 用于存储每个电机的最大速度


   // int32 trajectory_time[NUM_SLAVES]; // 当前轨迹时间
   // int32 trajectory[NUM_SLAVES][100]; // 用于存储每个电机的目标位置（角度）
   // my_float total_time[NUM_SLAVES];     // 轨迹总时间
...
//int32* quintic_trajectory(int32 pos0, int32 posf, my_float tf, int num_points);//五次多项式轨迹规划
...
void set_max_velocity(uint8 slave_index, int32 target_vel);//设置单个电机的最大速度
void set_max_acceleration(uint8 slave_index, int32 target_acc);//设置单个电机的最大加速度
void set_max_deceleration(uint8 slave_index, int32 target_decc);//设置单个电机的最大减速度
void read_max_velocity(void);


void get_cur_degree(void);//获取当前角度
int32 get_position_difference(void);//获取位置差
