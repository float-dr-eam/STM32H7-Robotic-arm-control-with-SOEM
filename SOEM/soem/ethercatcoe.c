 ...
 *
 * 该函数用于通过 CAN over EtherCAT (CoE) 协议从指定从站读取对象描述列表。
 * 它会向从站发送获取对象描述列表的请求，并处理从站返回的响应，
 * 将获取到的对象索引存储在提供的对象描述列表结构中。
 *
 * @param[in]  context  = 上下文结构体，包含与 EtherCAT 通信相关的信息。
 * @param[in]  Slave    = 从站编号，指定要与之通信的从站。
 * @param[out] pODlist  = 指向对象描述列表结构体的指针，用于存储从站返回的对象索引。
 * @return Workcounter of slave response. 从站响应的工作计数器值，若出错则可能为 0。
 */
int ecx_readODlist(ecx_contextt *context, uint16 Slave, ec_ODlistt *pODlist)
{
   // 定义指向 SDO 服务结构体的指针，分别用于发送和接收数据
   ec_SDOservicet *SDOp, *aSDOp;
   // 定义邮箱缓冲区，用于存储发送和接收的邮箱数据
   ec_mbxbuft MbxIn, MbxOut;
   // 工作计数器，用于记录从站响应的状态
   int wkc;
   // 循环变量、索引数量、循环计数器、存储位置、偏移量
   ...
   // 尝试从从站接收邮箱数据，若有则清空，超时时间设置为 0
   wkc = ecx_mbxreceive(context, Slave, &MbxIn, 0);
   // 清空发送邮箱缓冲区
   ec_clearmbx(&MbxOut);
   // 将接收邮箱缓冲区的地址转换为 SDO 服务结构体指针
   aSDOp = (ec_SDOservicet*)&MbxIn;
   // 将发送邮箱缓冲区的地址转换为 SDO 服务结构体指针
   ...
   // 更新从站的邮箱计数器值
   context->slavelist[Slave].mbx_cnt = cnt;
   // 设置发送邮箱头的类型为 CoE，并包含会话句柄信息
   SDOp->MbxHeader.mbxtype = ECT_MBXT_COE + (cnt << 4); /* CoE */
   // 设置 CANOpen 字段，指定服务类型为 SDO 信息服务
      ...
      do
      {
         ...
         if (wkc > 0)
         {
                ...
                ((aSDOp->Opcode & 0x7f) == ECT_GET_ODLIST_RES))
            {
               ...
               // 检查提取的索引数量是否超出缓冲区结构的最大容量
               if ((sp + n) > EC_MAXODLIST)
               {
                  // 调整索引数量，避免超出缓冲区容量
                  n = EC_MAXODLIST + 1 - sp;
                  // 报告 SDO 信息错误，提示主站缓冲区条目过多
               ...
               // 检查提取的索引数量是否超出对象描述列表的最大条目数
               if ((pODlist->Entries + n) > EC_MAXODLIST)
               {
                  // 调整索引数量，避免超出对象描述列表的最大条目数
...
/**
 * @brief 通过 CAN over EtherCAT (CoE) 协议进行阻塞式的 SDO 读取操作。支持单子索引读取或完整访问读取。
 * 
 * 该函数向从站发送 SDO 读取请求，仅会发起 “普通” 的上传请求。若请求的参数大小小于等于 4 字节，
 * 从站会返回 “快速” 响应；否则，返回 “普通” 响应。若 “普通” 响应的数据量大于邮箱大小，
 * 响应会被分段传输，函数会将所有分段数据合并并复制到参数缓冲区中。
 * 
 * @param[in]  slave      从站编号，指定要进行读取操作的从站。
 * @param[in]  index      要读取的对象字典索引。
 * @param[in]  subindex   要读取的子索引。若使用完整访问模式（CA），此值必须为 0 或 1。
 * @param[in]  CA         模式标志，FALSE 表示单子索引读取，TRUE 表示完整访问读取（即读取所有子索引）。
 * @param[in,out] psize   参数缓冲区的初始大小（单位：字节），函数执行完成后会更新为从 SDO 实际读取的字节数。
 * @param[out] p          指向参数缓冲区的指针，用于存储从从站读取的数据。
 * @param[in]  timeout    操作的超时时间（单位：微秒），标准值为 EC_TIMEOUTRXM。
 * @return 从站最后一次响应的工作计数器值，可用于判断操作是否成功。
 * @see ecx_SDOread 底层实现该功能的函数。
 */
...
/**
 * @brief 通过 CAN over EtherCAT (CoE) 协议进行 SDO 写操作，此操作是阻塞式的。可进行单子索引写入或完整访问写入。
 * 
 * 除非数据量较小，会使用 “快速” 传输方式，否则会发起一个 “普通” 的下载请求。
 * 如果要写入的参数大小超过邮箱大小，下载操作将被分段处理。该函数会将参数数据分割成多个段，逐个发送给从站。
 * 
 * @param[in]  Slave      从站编号，指定要进行写操作的从站。
 * @param[in]  Index      要写入的对象字典索引。
 * @param[in]  SubIndex   要写入的子索引。如果使用完整访问模式（CA），此值必须为 0 或 1。
 * @param[in]  CA         模式标志，FALSE 表示单子索引写入，TRUE 表示完整访问写入（即写入所有子索引）。
 * @param[in]  psize      参数缓冲区的大小，单位为字节。
 * @param[out] p          指向参数缓冲区的指针，包含要写入从站的数据。
 * @param[in]  Timeout    操作的超时时间，单位为微秒，标准值为 EC_TIMEOUTRXM。
 * @return 从站最后一次响应的工作计数器值。
 * @see ecx_SDOwrite
 */
...
/**
 * @brief 通过 CAN over EtherCAT (CoE) 协议进行阻塞式的 RxPDO 写入操作。
 * 
 * 该函数会发起一个 RxPDO 下载请求，将指定的 PDO 数据写入到指定从站的相关 RxPDO 中。
 * 
 * @param[in]  Slave         从站编号，指定要进行写入操作的从站。
 * @param[in]  RxPDOnumber   相关的 RxPDO 编号。
 * @param[in]  psize         PDO 缓冲区的大小，单位为字节。
 * @param[out] p             指向 PDO 缓冲区的指针，包含要写入从站的数据。
 * @return 从站最后一次响应的工作计数器值。
 * @see ecx_RxPDO
 */
...
/**
 * @brief 通过 CAN over EtherCAT (CoE) 协议进行阻塞式的 TxPDO 远程读取请求操作。
 * 
 * 该函数会发起一个TxPDO 读取请求，从指定从站的相关 TxPDO 中读取数据。
 * 
 * @param[in]  slave         从站编号，指定要进行读取操作的从站。
 * @param[in]  TxPDOnumber   相关的 TxPDO 编号。
 * @param[in,out] psize      PDO 缓冲区的大小，单位为字节，函数返回时会更新为实际从 PDO 读取的字节数。
 * @param[out] p             指向 PDO 缓冲区的指针，用于存储从从站读取的数据。
 * @param[in]  timeout       操作的超时时间，单位为微秒，标准值为 EC_TIMEOUTRXM。
 * @return 从站最后一次响应的工作计数器值。
 * @see ecx_TxPDO
 */
