...
/**
 * @brief       配置MPU的region(SRAM区域为透写模式)
 * @param       无
 * @retval      无
 */
...
void lcd_init(void)
{
    ...
    else
    {
        ...
        fmc_read_handle.DataSetupTime = 0x4E;             /* 数据保存时间(DATAST)为78个fmc_ker_ck = 4.5ns * 78 = 351ns */
                                                          /* 因为液晶驱动IC的读数据的时候,速度不能太快,尤其是个别奇葩芯片 */
        fmc_read_handle.AccessMode = FMC_ACCESS_MODE_A;   /* 模式A */
        /* FMC写时序控制寄存器 */
        fmc_write_handle.AddressSetupTime = 0x0F;         /* 地址建立时间(ADDSET)为15个fmc_ker_ck = 4.5ns * 15 = 67.5n */
        fmc_write_handle.AddressHoldTime = 0x00;
        fmc_write_handle.DataSetupTime = 0x0F;            /* 数据保存时间(DATAST)为15个fmc_ker_ck = 4.5ns * 15 = 67.5n */
        ...
        {
            /* 重新配置写时序控制寄存器的时序 */
            fmc_write_handle.AddressSetupTime = 4;  /* 地址建立时间(ADDSET)为4个fmc_ker_ck = 18 ns */
            fmc_write_handle.DataSetupTime = 4;     /* 数据保存时间(DATAST)为4个fmc_ker_ck = 18 ns */
        ...
        {
            /* 重新配置写时序控制寄存器的时序 */
            fmc_write_handle.AddressSetupTime = 7;  /* 地址建立时间(ADDSET)为7个fmc_ker_ck = 31.5 ns */
            fmc_write_handle.DataSetupTime = 7;     /* 数据保存时间(DATAST)为7个fmc_ker_ck = 31.5 ns */
            FMC_NORSRAM_Extended_Timing_Init(g_sram_handle.Extended, &fmc_write_handle, g_sram_handle.Init.NSBank, g_sram_handle.Init.ExtendedMode);
        }
        else if (lcddev.id == 0x5310 || lcddev.id == 0x5510 || lcddev.id == 0x7796 || lcddev.id == 0x9806)
        {
            /* 重新配置写时序控制寄存器的时序 */
