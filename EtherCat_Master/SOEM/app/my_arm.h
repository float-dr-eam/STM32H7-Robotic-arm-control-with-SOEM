...
//void arm_forward_kinematics2(const my_float degree[NUM_SLAVES], bool side);// 计算机械臂正运动学


void init_robot(Robot *robot) ;// 初始化机械臂
...
void vector_subtract(my_float *a, my_float *b, my_float *result, int n);// 向量减法
...
void forward_kinematics(Robot *robot, my_float *q, my_float *T_result);// 计算正向运动学
int inverse_kinematics ( Robot *robot,const my_float *T_desired, my_float *q_init, my_float *q_result, int max_iter, my_float epsilon, my_float lambda, my_float step_size, int verbose);// 逆运动学

void arm_forward_kinematics(const my_float *q_init);// 机械臂正运动学
void arm_inverse_kinematics(const my_float *T_desired, const my_float *q_init, my_float *q_result);// 机械臂逆运动学
