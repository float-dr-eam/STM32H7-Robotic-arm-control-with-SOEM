 ...
 * @brief 向从站传输过程数据。
 *
 * 根据 LRW 是否被允许（blockLRW 标志），使用 LRW 或 LRD/LWR 协议。
 * 同时传输输入和输出过程数据，输出数据为实际数据，输入数据为占位符。
 * 输入数据通过接收过程数据的函数收集。
 * 与基础 LRW 函数不同，此函数是非阻塞的。
 * 如果过程数据无法放入一个数据报中，则会使用多个数据报。
 * 为了重组从站的响应，使用了一个栈。
 *
 * @param[in]  context        上下文结构体，包含与 EtherCAT 通信相关的信息。
 * @param[in]  group          组编号，用于指定要操作的 EtherCAT 组。
 * @param[in]  use_overlap_io 标志位，指示是否使用重叠的 I/O 映射。
 * @return 如果过程数据成功传输，则返回大于 0 的值。
 */
static int ecx_main_send_processdata(ecx_contextt *context, uint8 group, boolean use_overlap_io)
{
   // 定义逻辑地址，用于指定数据传输的起始地址
   uint32 LogAdr;
   // 定义两个 16 位无符号整数，用于存储地址的低 16 位和高 16 位
   uint16 w1, w2;
   // 定义数据传输的总长度和分段长度
   int length, sublength;
   // 定义数据报的索引
   uint8 idx;
   // 定义工作计数器，用于表示操作的状态和结果
   int wkc;
   // 定义指向数据缓冲区的指针
   uint8 *data;
   // 定义标志位，指示是否为第一个数据报
   ...
   // 初始化工作计数器为 0
   wkc = 0;
   // 如果指定组支持分布式时钟（DC），则设置 first 标志为 TRUE
   ...
   if (use_overlap_io == TRUE)
   {
      ...
      // 保存输入 I/O 映射的偏移量，用于补偿返回帧中输入数据的存储位置
      iomapinputoffset = context->grouplist[group].Obytes;
   }
   else
   {
      // 否则，数据传输的总长度为输出字节数和输入字节数之和
   ...
   if (length)
   {
      ...
      else
      {
         ...
         else
         {
            // 否则，获取输入数据的缓冲区指针
            data = context->grouplist[group].inputs;
            // 清除输入 I/O 映射的偏移量，因为如果只有输入数据，则不需要补偿重叠的 I/O 映射
            iomapinputoffset = 0;
         }
         /* segment transfer if needed */
         // 如果需要分段传输数据
         do
         {
             ...
             */
            // 将数据报索引和数据指针压入栈中，iomapinputoffset 用于补偿重叠 I/O 映射中输入数据的存储位置
 ...
 * 从从站接收过程数据。
 *
 * 此函数是 `ec_send_processdata()` 函数的第二部分。借助栈将接收到的数据报与过程数据重新组合。
 * 如果数据报包含输入过程数据，则将其复制到过程数据结构中。
 *
 * @param[in]  context  上下文结构体，包含与 EtherCAT 通信相关的信息。
 * @param[in]  group    组编号，用于指定要操作的 EtherCAT 组。
 * @param[in]  timeout  超时时间，单位为微秒。若在该时间内未完成操作，则停止等待。
 * @return 工作计数器的值，用于表示操作的状态和结果。
 */
