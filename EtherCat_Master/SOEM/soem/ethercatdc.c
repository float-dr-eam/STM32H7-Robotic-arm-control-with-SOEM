...
/**
 * @brief 设置指定从站的分布式时钟（DC），使其以指定的周期时间触发 SYNC0 信号，并可设置偏移量。
 * 
 * 该函数用于配置 EtherCAT 从站的分布式时钟，控制 SYNC0 信号的触发。
 * 可以激活或停用 SYNC0 信号的周期性触发，并指定触发的周期时间和偏移量。
 * 
 * @param[in]  context        指向 EtherCAT 上下文结构体的指针，包含了 EtherCAT 主站和从站的各种配置信息和状态。
 * @param [in] slave          从站编号，用于指定要配置的 EtherCAT 从站。
 * @param [in] act            布尔值，TRUE 表示激活 SYNC0 信号的周期性触发，FALSE 表示停用。
 * @param [in] CyclTime       周期时间，单位为纳秒（ns），指定 SYNC0 信号触发的周期。
 * @param [in] CyclShift      偏移量，单位为纳秒（ns），用于调整 SYNC0 信号首次触发的时间。
 * 
 * @note 此函数会与从站进行通信，通过写入特定的寄存器来配置分布式时钟。
 *       若 `CyclTime` 大于 0，首次触发时间会计算为一个完整的周期时间的倍数加上偏移量；
 *       若 `CyclTime` 等于 0，首次触发时间为当前时间加上固定延迟和偏移量。
 */
...
/**
 * 定位具有分布式时钟(DC)功能的从站,并测量传播延迟。
 *
 * @param[in]  context        指向 EtherCAT 上下文结构体的指针
 * @return boolean 如果找到具有 DC 功能的从站则返回 TRUE,否则返回 FALSE
 */
