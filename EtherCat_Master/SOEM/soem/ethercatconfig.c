...
static int ecx_map_coe_soe(ecx_contextt *context, uint16 slave, int thread_n)//
{
   ...
   if(context->slavelist[slave].PO2SOconfig) /* only if registered */ /* 执行特殊的从机配置钩子，从Pre-Op到Safe-Op */
   {
      context->slavelist[slave].PO2SOconfig(slave);   /* 从机配置钩子，从Pre-Op到Safe-Op */
   }
   if (context->slavelist[slave].PO2SOconfigx) /* only if registered */ /* 执行特殊的从机配置钩子，从Pre-Op到Safe-Op */
   {
      context->slavelist[slave].PO2SOconfigx(context, slave);   /* 从机配置钩子，从Pre-Op到Safe-Op */
   }
...
#else
            /* serialised version */
            ecx_map_coe_soe(context, slave, 0);    //ec_slave[cnt].PO2SOconfig
#endif
      }
   }
...
/**
 * @brief 将一组从站的所有过程数据对象（PDO）映射到 IO 映射区域，输出和输入按顺序排列（SOEM 传统方式）。
 * 
 * 此函数会完成一系列操作，包括查找从站的映射关系、配置同步管理器（Sync Manager, SM）、
 * 创建输出和输入的现场总线内存管理单元（FMMU）映射，最终将从站的 PDO 数据映射到指定的 IO 映射区域。
 * 
 * @param[in]  context    指向 EtherCAT 上下文结构体的指针，包含了 EtherCAT 主站和从站的各种配置信息和状态。
 * @param[out] pIOmap     指向 IO 映射区域的指针，用于存储从站的输出和输入数据。
 * @param[in]  group      要进行映射的从站组编号，0 表示所有组。
 * @return  返回 IO 映射区域的大小，单位为字节。如果没有有效的从站或者组编号无效，则返回 0。
 */
int ecx_config_map_group(ecx_contextt *context, void *pIOmap, uint8 group)
{
   ...
   if ((*(context->slavecount) > 0) && (group < context->maxgroup))
   {
      ...
      /* Find mappings and program syncmanagers */// 查找映射和配置同步管理器
      ecx_config_find_mappings(context, group);//会调用   ec_slave[cnt].PO2SOconfig
