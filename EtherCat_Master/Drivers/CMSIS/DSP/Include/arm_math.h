...
arm_status arm_mat_mult_fast_q31(
  ...
  /**
 * @brief 浮点矩阵减法
 * @param[in]  pSrcA  指向第一个输入矩阵结构的指针
 * @param[in]  pSrcB  指向第二个输入矩阵结构的指针 
 * @param[out] pDst   指向输出矩阵结构的指针
 * @return     函数根据大小检查的结果返回
 * <code>ARM_MATH_SIZE_MISMATCH</code>或<code>ARM_MATH_SUCCESS</code>
...
__STATIC_FORCEINLINE q15_t arm_pid_q15(
  ...
  /**
 * @brief 浮点矩阵求逆。
 * @param[in]  src   指向输入浮点矩阵结构体实例的指针。
 * @param[out] dst   指向输出浮点矩阵结构体实例的指针。
 * @return 如果维度不匹配,函数返回 ARM_MATH_SIZE_MISMATCH。
 * 如果输入矩阵是奇异的(没有逆矩阵),则算法终止并返回错误状态 ARM_MATH_SINGULAR。
...
/**
 * @brief  浮点数据的三角正弦函数快速近似计算
 * @param[in] x  输入值,单位为弧度
 * @return  sin(x)的值
 */   
...
/** 
 * @brief  Q31数据的三角正弦函数快速近似计算
 * @param[in] x  输入值,单位为弧度,已缩放
 * @return  sin(x)的值
 */ 
  q31_t arm_sin_q31(
  q31_t x);
