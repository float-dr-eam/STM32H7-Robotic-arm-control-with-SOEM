...
//TCM : Tightly-Coupled Memory 紧密耦合内存 。ITCM用于指令，DTCM用于数据，特点是跟内核速度一样
/* 内存管理表 */
...
static __ALIGNED(64) uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((section(".bss.ARM.__at_0XC01F4000")));    /* 外部SDRAM内存池,前面2M给LTDC用了(1280*800*2) */
static __ALIGNED(64) uint8_t mem3base[MEM3_MAX_SIZE] __attribute__((section(".bss.ARM.__at_0x30000000")));    /* 内部SRAM1+SRAM2内存池 */
static __ALIGNED(64) uint8_t mem4base[MEM1_MAX_SIZE] __attribute__((section(".bss.ARM.__at_0x38000000")));    /* 内部SRAM4内存池 */
static __ALIGNED(64) uint8_t mem5base[MEM2_MAX_SIZE] __attribute__((section(".bss.ARM.__at_0x20000000")));    /* 内部DTCM内存池 */
static __ALIGNED(64) uint8_t mem6base[MEM3_MAX_SIZE] __attribute__((section(".bss.ARM.__at_0x00000000")));    /* 内部ITCM内存池 */

/* 内存管理表 */
static MT_TYPE mem1mapbase[MEM1_ALLOC_TABLE_SIZE];                                                            /* 内部SRAM内存池MAP */
static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".bss.ARM.__at_0XC1E30000")));       /* 内部CCM内存池MAP */
static MT_TYPE mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((section(".bss.ARM.__at_0X3003C000")));       /* 外部SRAM内存池MAP */
static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".bss.ARM.__at_0X3800F000")));       /* 内部CCM内存池MAP */
static MT_TYPE mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((section(".bss.ARM.__at_0X2001E000")));       /* 外部SRAM内存池MAP */
static MT_TYPE mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".bss.ARM.__at_0X0000F000")));       /* 内部CCM内存池MAP */
...
{  
    my_mem_set(mallco_dev.memmap[memx], 0, memtblsize[memx] * 4);  /* 内存状态表数据清零 *///memtblsize[memx] * 4= 块数*u32/u8 = 块数*4字节
...
{  
    ...
    if (size % memblksize[memx]) nmemb++;        //如果不能整除，就多分配一个
    ...
    for (offset = memtblsize[memx] - 1;offset >= 0; offset--)   /* 搜索整个内存控制区 */
    {
        ...
        if (cmemb == nmemb)                                     /* 找到了连续nmemb个空内存块 */
        {
            ...
            {  
                mallco_dev.memmap[memx][offset + i] = nmemb;    /* 标注内存块非空， 给map中标注这片内存块使用的块数 = nmwmb*/
            }
...
uint8_t my_mem_free(uint8_t memx, uint32_t offset)
{
    ...
    {
        int index = offset / memblksize[memx];      /* 偏移所在内存块号码 */    //    偏移/64= 块号
        int nmemb = mallco_dev.memmap[memx][index]; /* 内存块数量 */        //获取map中第index个块的块数
