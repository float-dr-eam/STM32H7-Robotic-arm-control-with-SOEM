...
char *usmart_get_input_string(void)
{
    ...
    if (USART_RX_STA & 0x8000)        /* 串口接收完成？ */
    {
        ...
        receivedBuff[len] = '\0';     /* 在末尾加入结束符. */
        pbuf = (char *)receivedBuff;
        // printf("%s", pbuf);           /* 打印接收到的数据 */
        USART_RX_STA = 0;             /* 开启下一次接收 */
        HAL_UARTEx_ReceiveToIdle_DMA(&huart1,receivedBuff,receivedBuff_Length);	//重新开启串口空闲中断和DMA接收
    }
